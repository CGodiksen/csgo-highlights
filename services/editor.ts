...
const createHighlightVideo = (vodFolder: string, hightlightSpecifications: HighlightSpecification[]): void => {
    ...
    const highlightOrderFiles = await Promise.all(hightlightSpecifications.map(spec => ))
...
const getRelatedVod = async (vodFolder: string, hightlightSpecification: HighlightSpecification): Promise<string> => {
